...
func ADX(candles []Candle, period int) float64 {
	...
	// Return a simple approximation of ADX as smoothed DX
	return dx
}
...
// CVDDiverges returns true when price makes a new extreme but CVD doesn't confirm.
// bullish divergence: price lower low, CVD higher low → expect reversal up
// bearish divergence: price higher high, CVD lower high → expect reversal down
