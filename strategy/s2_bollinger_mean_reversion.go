...
// Edge:       Fades overextension within confirmed ranges — mean reversion
//             with institutional order book support.
...
func (s *BollingerMeanReversion) Evaluate(ctx MarketContext) Signal {
	...
	// ── 15m BB width percentile: must be < 40th (ranging, not expanding) ─────
	bbWidthPct := BBWidthPercentile(ctx.Candles15m, 20, 50)
	if bbWidthPct > 0.40 {
		return NoSignal(name) // bands expanding — not a range
	}

	// ── 5m Bollinger Bands and RSI ────────────────────────────────────────────
	...
	// ── Entry: LONG (price at lower band, oversold, bid wall, CVD reversing) ─
	atLowerBand := price <= bb5m.Lower+0.1*atr5m
	rsiOversold := rsi5m < 35
	bidWallPresent := ob.BidWallSize > ob.AskWallSize*1.5 // bids dominate
	cvdExhaustedSell := ctx.CVD >= ctx.CVDPrev            // selling pressure slowing
	...
	// ── Entry: SHORT (price at upper band, overbought, ask wall, CVD slowing) ─
