...
// Real-data gate.
//
// This desk exists to find strategies worth real money on the Live Engine. That
// makes the integrity of its record the whole product: a trade recorded against
...
func (g *feedGate) SetFeedStatus(live bool, source string) {
	...
	case !live && !g.loggedDown.Swap(true):
		log.Printf("[OPTIONS] ⛔ no real market price (source=%q) — managing open positions, opening nothing new. "+
	...
	case live && !prevLive:
		...
		log.Printf("[OPTIONS] ✅ real market price restored (source=%q) — entries re-enabled", source)
	}
}
