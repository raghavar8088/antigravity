...
//
// The old construction — long PUT mirroring a long CALL — failed this in the
// most common case of all: a flat market, where both halves decayed and both
// lost.
...
func TestMirror_SellsTheSameContract(t *testing.T) {
	...
	if !m.ShortPremium {
		t.Error("mirror is not short; a long mirror is a second bet, not an inverse")
	}
	if o.ShortPremium {
		t.Error("the original was marked short")
	}
}
...
// decays. A long call and a long put both lose here. One long and one short of
...
func TestMirror_PairIsCashNeutralAtOpen(t *testing.T) {
	...
	before := e.balance
	e.balance -= pos.CostBasis // what the caller does for the long leg
	orig.position = pos
	e.openMirrorLocked(orig, pos, now)

	if math.Abs(e.balance-before) > 1e-9 {
		t.Errorf("balance moved by %v opening a pair; the premium paid by one half is received by the other",
			e.balance-before)
	}
}
