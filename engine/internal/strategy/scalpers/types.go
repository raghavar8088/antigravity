...
type MarketContext struct {
	...
	// ── S30-S79 shadow strategy family feeds ───────────────────────────────

	// ETHPrice — Binance ETHUSDT spot/perp price, polled independently of the
	// primary BTC feed (S41 cointegration spread, S65 lead-lag confirmation).
	ETHPrice          float64
	ETHPricePopulated bool
	ETHPriceHealthy   bool

	// OIHistory — rolling open-interest readings (oldest first, up to 8) for
	// OI-velocity strategies (S75). OpenInterest/OpenInterestPrev above remain
	// the single-step view used by existing strategies.
	OIHistory []float64
