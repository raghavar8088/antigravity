...
// # What was wrong
//
// An ANTI_ strategy was built by flipping the option TYPE — a short CALL's
// mirror was a short PUT — and swapping the exits. Both halves stayed SHORT
// PREMIUM.
//
// That is not an inverse. A short call and a short put share the sign of theta
// and vega: both collect decay, both are hurt when volatility rises. In a flat
// market BOTH win, which is the opposite of what a mirror is for — the pair was
// supposed to show that if the original loses, this one gains. It fails worst in
// exactly the quiet conditions where most of these strategies trade.
//
// # What replaces it
//
// The exact inverse of "sell this contract at premium P" is "BUY THIS SAME
...
//
// This is the same correction the scalp desk and the buying desk needed, for the
// same underlying reason: a mirror that makes its own decisions is not a mirror.
// On the scalp desk it was competing for its own fill; here it was holding a
...
func (e *Engine) openMirrorLocked(orig *strategyState, pos *OptionPosition, now time.Time) {
	if orig == nil || pos == nil || pos.LongPremium {
		return
	}
	...
	// Buying the contract pays the premium. The original was credited the same
	// amount when it sold, so the pair is cash-neutral at open.
...
// short-premium rules, and re-deriving a long-side equivalent would make the two
// halves close at different premiums on different ticks, which is precisely what
// stops them being an inverse. Inheriting the exit keeps the P&L exactly
// negated.
//
...
func (e *Engine) closeMirrorLocked(orig *strategyState, reason string, now time.Time) {
	if orig == nil || orig.position == nil || orig.position.LongPremium {
		return
	}
	name := mirrorNameFor(orig.def.Name)
	if name == "" {
...
func (e *Engine) logMirrorImbalance(name string) {
	log.Printf("[OPTIONS SELLING] ⚠️  mirror %s could not open — the pair has drifted and its combined P&L is no longer -fees", name)
}
