...
const initialOptionsBalance = 1000000.0 // $1,000,000 paper account
...
func (e *Engine) Run(stopCh <-chan struct{}) {
	...
	log.Printf("[OPTIONS SELLING] %s started with %d strategies (tick every %v)", e.marketProfile.Name, len(e.states), interval)
...
func (e *Engine) maybeOpenLivePositionLocked(s *strategyState, ctx SignalContext, regime string, iv float64, now time.Time, openCount *int) {
	...
	if !e.paperDeskAggressiveOpen() {
		...
		if s.consecutiveLosses == 0 && s.stats.TotalTrades > 0 {
			if s.stats.Wins >= 2 && s.stats.Losses == 0 {
				cooldownMul = cooldownStreakWinMultiplier
			} else {
				cooldownMul = cooldownWinMultiplier
			}
		} else if s.consecutiveLosses > 0 {
			cooldownMul = cooldownLossMultiplier
		}
	...
	if !e.paperDeskAggressiveOpen() {
		...
		if !ok {
			log.Printf("[OPTIONS_SELLING] WARN: unknown signal %q for strategy %q — skipping", s.def.Signal, s.def.Name)
			return
		}
		if !fn(ctx) {
			return
...
func (e *Engine) newOptionPositionLocked(def StrategyDef, positionUSD, iv float64, now time.Time, prefix string) *OptionPosition {
	...
	var strike float64
	// SELLING logic: Sell Puts on Bullish signals, Sell Calls on Bearish signals.
	// We use StrategyDef.Type to determine if it's a Call or Put being SOLD.
	if def.Type == Call {
	...
	id := fmt.Sprintf("%s-%04d-%s", prefix, e.tradeSeq, nameTag)
...
func (e *Engine) markToMarketPositionLocked(pos *OptionPosition, iv, takeProfitPct, stopLossPct float64, now time.Time) string {
	...
	// ── Upgrade: Theta-Decay Acceleration ──
	effectiveTP := takeProfitPct
	switch {
	case timeProgress >= thetaAccelPhase2Progress:
		effectiveTP = takeProfitPct * thetaAccelPhase2TPScale
	case timeProgress >= thetaAccelPhase1Progress:
		effectiveTP = takeProfitPct * thetaAccelPhase1TPScale
	}

	profitLockThreshold := math.Max(optionLateExitMinGain, effectiveTP*optionProfitLockShareOfTarget)
	trailActivation := math.Max(optionLateExitMinGain, effectiveTP*0.36)
...
func (e *Engine) refreshStrategyPresentationLocked(s *strategyState, now time.Time) {
	...
	case !s.disabledUntil.IsZero() && now.Before(s.disabledUntil):
		s.stats.Status = optionStatusDisabled
	case s.stats.RosterState == StrategyRosterActive && !s.lastTradeAt.IsZero() && now.Sub(s.lastTradeAt) < time.Duration(s.def.CooldownSecs)*time.Second:
