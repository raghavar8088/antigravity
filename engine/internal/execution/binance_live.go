...
func (b *BinanceLiveClient) ExecuteSignal(sig strategy.Signal, mode OrderMode) (FillResult, error) {
	log.Printf("[LIVE EXEC] Attempting %s %s on Binance for %.4f %s", mode, sig.Action, sig.TargetSize, sig.Symbol)
	if mode == OrderModePostOnly {
		return FillResult{}, fmt.Errorf("post-only live routing requires a priced limit order and is not wired yet")
	}
	...
	if err != nil {
		return FillResult{}, fmt.Errorf("failed to sign order request: %w", err)
	}
	...
	if err != nil {
		return FillResult{}, fmt.Errorf("NETWORK ERROR: Execution layer failed to reach Binance: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return FillResult{}, fmt.Errorf("EXCHANGE ERROR: Binance rejected order with HTTP %d", resp.StatusCode)
	}

	log.Printf("[LIVE EXEC] SUCCESS! Routed %s %.4f %s to Binance.", mode, sig.TargetSize, sig.Symbol)
	return FillResult{
		ExecPrice: 0,
		OrderMode: mode,
	}, nil
...
func (b *BinanceLiveClient) PlaceMarketOrder(sig strategy.Signal) error {
	_, err := b.ExecuteSignal(sig, OrderModeMarket)
	return err
}
