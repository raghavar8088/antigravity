  ...
  const refresh = useCallback(async () => {
    try {
      const [h, s, lb, tr, po] = await Promise.all([
        fetch("/api/scalp-highvol/scalp/health", { cache: "no-store" }),
        fetch("/api/scalp-highvol/scalp/stats", { cache: "no-store" }),
        fetch("/api/scalp-highvol/scalp/leaderboard", { cache: "no-store" }),
        fetch(`/api/scalp-highvol/scalp/trades?n=${showAllTrades ? 5000 : 50}`, { cache: "no-store" }),
        fetch("/api/scalp-highvol/scalp/positions", { cache: "no-store" }),
      ]);
      ...
      <main className="desk-page">
        ...
        <div>
          ...
          <p className="desk-body-md" style={{ marginTop: 6, maxWidth: 720, color: "var(--desk-on-surface-variant)" }}>
            The same scalp strategies as the Crypto Scalp Desk, on the highest-volume currencies only —
        ...
        <DeskAdminControls
          resetPath="/api/scalp-highvol/scalp/reset"
          clearPath="/api/scalp-highvol/scalp/clear-trades"
