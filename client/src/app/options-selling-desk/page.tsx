 ...
 *
 * Styling matches the TradingAI terminal, mirroring Crypto Scalp Desk: clean
 * white rounded-2xl cards, uppercase labels, large monospace numbers, violet
 * primary, amber CTA. Reads only through the session-gated /api/options-selling
 * proxy. Paper only — short options against the engine's synthetic BTC chain,
 * no real money.
 */
...
const pillBase = "cursor-pointer rounded-full border px-3.5 py-1.5 text-[12px] font-semibold transition-colors";
    ...
    <main className="mx-auto flex max-w-6xl flex-col gap-6 px-8 py-8">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <StatCard
          ...
          sub={<span className="text-zinc-400">short option positions live</span>}
        />
        <StatCard
          label="Active Roster"
          value={strategies.length ? `${activeCount} / 13` : "—"}
          sub={<span className="text-zinc-400">of {strategies.length} strategies, capped 4 per category</span>}
        />
      </div>
              ...
              {filtered.map((r) => (
                <tr key={r.strategyId} className="border-b border-zinc-50 transition-colors hover:bg-zinc-50">
                  ...
                  <td className="px-3 py-2.5">
                    <span className={`text-[11px] font-bold ${r.optionType === "PUT" ? "text-emerald-600" : "text-red-600"}`}>{r.optionType}</span>
                  </td>
            ...
            <tbody style={mono}>
              ...
              {positions.map((p) => (
                <tr key={p.id} className="border-b border-zinc-50 transition-colors hover:bg-zinc-50">
                  ...
                  <td className="px-3 py-2.5">
                    <span className={`text-[11px] font-bold ${p.optionType === "PUT" ? "text-emerald-600" : "text-red-600"}`}>{p.optionType}</span>
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-right tabular-nums text-zinc-500">{fmtUSD(p.marginBlocked)}</td>
            ...
            <tbody style={mono}>
              ...
              {trades.slice(0, 150).map((t) => (
                <tr key={t.id} className="border-b border-zinc-50 transition-colors hover:bg-zinc-50">
                  ...
                  <td className="px-3 py-2.5">
                    <span className={`text-[11px] font-bold ${t.optionType === "PUT" ? "text-emerald-600" : "text-red-600"}`}>{t.optionType}</span>
                  </td>
