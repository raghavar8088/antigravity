...
/**
 * Crypto Options Buying Desk — dashboard for the options (long-premium)
 * paper engine (50 BTC option-buying strategies: scalp / intraday / swing).
 *
 * Built on the shared desk primitives (components/desk/ui) and --desk-*
 * tokens, mirroring Crypto Options Selling. Reads only through the
 * session-gated /api/options-buying proxy. Paper only — long calls/puts
 * against the engine's synthetic BTC chain, no real money.
 */
...
export default function OptionsBuyingDeskPage() {
  ...
  const strategyColumns: DeskColumn<StrategyStatus>[] = [
    ...
    { id: "category", header: "Category", cell: (r) => r.category },
    {
      ...
      header: "Type",
      cell: (r) => (
        <span style={{ fontSize: "0.6875rem", fontWeight: 700, color: r.optionType === "CALL" ? "var(--desk-success)" : "var(--desk-error)" }}>
          {r.optionType}
        </span>
      ),
    },
  ...
  const positionColumns: DeskColumn<OptionPosition>[] = [
    ...
    {
      id: "type", header: "Type",
      cell: (p) => <span style={{ fontSize: "0.6875rem", fontWeight: 700, color: p.optionType === "CALL" ? "var(--desk-success)" : "var(--desk-error)" }}>{p.optionType}</span>,
    },
    ...
    { id: "costBasis", align: "right", header: "Cost Basis", cell: (p) => fmtUSD(p.costBasis) },
  ...
  const tradeColumns: DeskColumn<OptionTrade>[] = [
    ...
    {
      id: "type", header: "Type",
      cell: (t) => <span style={{ fontSize: "0.6875rem", fontWeight: 700, color: t.optionType === "CALL" ? "var(--desk-success)" : "var(--desk-error)" }}>{t.optionType}</span>,
    },
      ...
      <main className="desk-page">
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "0.8125rem" }}>
            ...
            <span className="desk-body-md" style={{ fontWeight: 500 }}>Options Buying Desk</span>
          </div>
          <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", alignItems: "center", gap: 12 }}>
            <h1 className="desk-display-lg" style={{ fontSize: "2rem" }}>Crypto Options Buying</h1>
        ...
        <div className="desk-metrics-row">
          ...
          <DeskMetricTile
            ...
            sub="long option positions live"
          />
          <DeskMetricTile
            label="Active Roster"
            value={strategies.length ? `${activeCount} / ${strategies.length}` : "—"}
            sub={`of ${strategies.length} strategies, capped per category`}
          />
        </div>
