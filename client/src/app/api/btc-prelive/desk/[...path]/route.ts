/**
 * BTC Pre-Live Engine Proxy (Phase 3 — $1,000 paper desk)
 *
 * Forwards authenticated requests to the BTC pre-live instance (a second
 * cmd/pre_live process with its own whitelist/balance/account key) running on
 * BTC_PRE_LIVE_ENGINE_URL (default http://127.0.0.1:8092).
 *
...
function upstreamBase(): string {
  return (
    process.env.BTC_PRE_LIVE_ENGINE_URL?.trim().replace(/\/+$/, "") ??
    "http://127.0.0.1:8092"
  );
}
...
async function handle(req: NextRequest, ctx: RouteCtx): Promise<NextResponse> {
  ...
  if (!isAllowed(pathname)) {
    return NextResponse.json(
      { ok: false, error: `${pathname} is not available via the BTC pre-live proxy` },
      { status: 403 },
    );
  }
