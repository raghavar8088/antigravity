/**
 * /api/mcx/state — Persist and restore MCX engine state
 */
...
async function ensureTable(client: { query: (sql: string) => Promise<unknown> }) {
  await client.query(`
    CREATE TABLE IF NOT EXISTS mcx_state (
  ...
  `);
  await client.query(`
    INSERT INTO mcx_state (id) VALUES (1)
  ...
  try {
    ...
    try {
      ...
      const { rows } = await client.query<{
        ...
        saved_at: string;
      }>(`SELECT * FROM mcx_state WHERE id = 1`);
      ...
      return NextResponse.json({
        ...
        state: {
          ...
          commodities: JSON.parse(row.commodities_json) as unknown[],
    ...
    console.error("[mcx/state GET]", err);
...
export async function POST(request: Request) {
  try {
    ...
    console.error("[mcx/state POST]", err);
    return NextResponse.json({ ok: false, error: String(err) }, { status: 500 });
  }
}
