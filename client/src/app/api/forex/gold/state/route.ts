...
async function ensureTable(client: { query: (sql: string, params?: unknown[]) => Promise<{ rows?: unknown[] }> }) {
  await client.query(`
    CREATE TABLE IF NOT EXISTS forex_gold_state (
  ...
  `);
  await client.query(`
    INSERT INTO forex_gold_state (id) VALUES (1)
  ...
  try {
    ...
    try {
      ...
      const { rows } = await client.query<{
        ...
        saved_at: string;
      }>(`SELECT * FROM forex_gold_state WHERE id = 1`);
    ...
    console.error("[forex/gold/state GET]", error);
...
export async function POST(request: Request) {
  try {
    ...
    console.error("[forex/gold/state POST]", error);
    return NextResponse.json({ ok: false, error: String(error) }, { status: 500 });
  }
}
