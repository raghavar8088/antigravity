/**
 * /api/crypto/equity-state — Persist and restore Crypto Equity engine state
 *
 * GET  — load last saved state
 * POST — save current engine snapshot
 *
 * Uses the shared DATABASE_URL Postgres instance.
 * Table: crypto_equity_state (auto-created on first call)
 */
...
async function ensureTable(client: { query: (sql: string) => Promise<unknown> }) {
  await client.query(`
    CREATE TABLE IF NOT EXISTS crypto_equity_state (
  ...
  `);
  await client.query(`
    INSERT INTO crypto_equity_state (id) VALUES (1)
  ...
  try {
    ...
    try {
      ...
      const { rows } = await client.query<{
        ...
        saved_at: string;
      }>(`SELECT * FROM crypto_equity_state WHERE id = 1`);
    ...
    console.error("[crypto/equity-state GET]", err);
...
export async function POST(request: Request) {
  try {
    ...
    try {
      await ensureTable(client);
      await client.query(
        `UPDATE crypto_equity_state SET
          balance         = $1,
          total_wins      = $2,
          total_losses    = $3,
          total_pnl       = $4,
    ...
    console.error("[crypto/equity-state POST]", err);
    return NextResponse.json({ ok: false, error: String(err) }, { status: 500 });
  }
}
