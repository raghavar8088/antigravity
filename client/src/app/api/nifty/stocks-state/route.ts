/**
 * /api/nifty/stocks-state — Persist and restore NIFTY 50 stocks engine state
 *
 * GET  — load last saved state
 * POST — save current engine snapshot
 *
 * Uses the same DATABASE_URL as the Go engine and the options engine.
 * Table: nifty_stocks_state (auto-created on first call)
 */
...
async function ensureTable(client: { query: (sql: string) => Promise<unknown> }) {
  await client.query(`
    CREATE TABLE IF NOT EXISTS nifty_stocks_state (
  ...
  `);
  await client.query(`
    INSERT INTO nifty_stocks_state (id) VALUES (1)
  ...
  try {
    ...
    try {
      ...
      const { rows } = await client.query<{
        balance: string; total_wins: string; total_losses: string;
        total_pnl: string; trade_seq: string;
        trades_json: string; strategies_json: string; saved_at: string;
      }>(`SELECT * FROM nifty_stocks_state WHERE id = 1`);
    ...
    console.error("[nifty/stocks-state GET]", err);
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      await client.query(
        `UPDATE nifty_stocks_state SET
         ...
         WHERE id = 1`,
        [
          body.balance, body.totalWins, body.totalLosses,
          body.totalPnl, body.tradeSeq,
    ...
    console.error("[nifty/stocks-state POST]", err);
    return NextResponse.json({ ok: false, error: String(err) }, { status: 500 });
  }
}
