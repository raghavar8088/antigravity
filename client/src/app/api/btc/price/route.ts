...
async function fetchCoinbase(): Promise<PriceData | null> {
  try {
    // Coinbase Advanced Trade stats — includes 24h high/low/change
    ...
    if (statsRes.status === "fulfilled" && statsRes.value.ok) {
      const s = await statsRes.value.json() as {
        high: string; low: string; volume: string; open: string; last: string;
      };
...
export async function GET() {
  // Try Binance first (full 24h stats in one call)
  const binance = await fetchBinance();
  if (binance) {
    return NextResponse.json({ ok: true, source: "binance", ...binance }, {
      headers: { "Cache-Control": "no-store, max-age=0" },
    });
  }

  // Binance blocked/down — use Coinbase (spot + exchange stats)
  const coinbase = await fetchCoinbase();
  if (coinbase) {
    return NextResponse.json({ ok: true, source: "coinbase", ...coinbase }, {
      headers: { "Cache-Control": "no-store, max-age=0" },
    });
  }
