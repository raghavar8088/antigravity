/**
 * /api/btc/spot-state — Persist and restore BTC spot scalper state
 */
...
async function ensureTable(client: { query: (sql: string) => Promise<unknown> }) {
  await client.query(`
    CREATE TABLE IF NOT EXISTS btc_spot_state (
      id              INTEGER PRIMARY KEY DEFAULT 1,
      balance         DOUBLE PRECISION NOT NULL DEFAULT 10,
  ...
  `);
  await client.query(`
    INSERT INTO btc_spot_state (id) VALUES (1)
...
function normalizePayload(body: Partial<Payload>): Payload {
  return {
    balance: toFiniteNumber(body.balance, 10),
    ...
    trades: Array.isArray(body.trades) ? body.trades.slice(0, 2000) : [],
  ...
  try {
    ...
    try {
      ...
      const { rows } = await client.query<{
        ...
        saved_at: string;
      }>(`SELECT * FROM btc_spot_state WHERE id = 1`);
      ...
      return NextResponse.json({
        ...
        found: true,
        state: {
          balance: toFiniteNumber(row.balance, 10),
    ...
    console.error("[btc/spot-state GET]", err);
...
export async function POST(request: Request) {
  try {
    ...
    console.error("[btc/spot-state POST]", err);
    return NextResponse.json({ ok: false, error: String(err) }, { status: 500 });
  }
}
