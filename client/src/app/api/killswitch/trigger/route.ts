 ...
 * Body: { reason?: string }
 *
 * Proxies to Go engine: POST /api/admin/ks/block
 * Requires ENGINE_ADMIN_SECRET env var on the server.
 *
 * Expected engine response shape:
 * { ok: boolean; message: string; triggeredAt: string }
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${ENGINE_BASE}/api/admin/ks/block`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Engine-Admin-Secret": adminSecret,
        "X-Service-Name": "vercel-ks-trigger",
