...
export default function PositionsTable({ currentPrice }: { currentPrice: number }) {
  ...
  const positions = [
    { id: "POS-183012-1", symbol: "BTCUSDT", side: "LONG", entry: 63850.10, size: 0.15, slPct: 0.5, tpPct: 1.0, strategy: "Bollinger_Squeeze_Scalp" },
    { id: "POS-183024-2", symbol: "BTCUSDT", side: "LONG", entry: 64100.00, size: 0.05, slPct: 0.5, tpPct: 1.0, strategy: "EMA_Cross_Scalp" },
    { id: "POS-183105-3", symbol: "BTCUSDT", side: "LONG", entry: 65200.50, size: 0.01, slPct: 0.3, tpPct: 0.8, strategy: "OrderFlow_Imbalance_Scalp" },
  ];
      ...
      <table className="w-full text-left text-sm">
        ...
        <tbody>
          {positions.map((p, i) => {
            ...
            const stopLoss = p.side === "LONG" 
              ? p.entry * (1 - p.slPct / 100) 
              : p.entry * (1 + p.slPct / 100);
            const takeProfit = p.side === "LONG"
              ? p.entry * (1 + p.tpPct / 100)
              : p.entry * (1 - p.tpPct / 100);
            const unrealizedPnL = p.side === "LONG"
              ? (markPrice - p.entry) * p.size
              : (p.entry - markPrice) * p.size;
            const unrealizedPct = ((markPrice - p.entry) / p.entry) * 100;

            // Calculate how close price is to SL or TP (for progress bar)
              ...
              <tr key={i} className="border-b border-gray-800/50 hover:bg-white/5 transition-colors group">
                ...
                <td className="py-4 px-2">
                  <div className="flex flex-col gap-1">
                    ...
                    {/* Visual progress bar: SL ← price → TP */}
