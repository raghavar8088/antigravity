    ...
    <div className="min-h-screen bg-[#080b10] text-zinc-100">
      <header className="sticky top-0 z-40 border-b border-zinc-800 bg-[#0b0f16]/95 backdrop-blur">
        <div className="flex min-h-[54px] flex-wrap items-center gap-3 px-3 lg:px-4">
          ...
          <div className="font-mono text-lg font-semibold text-zinc-50">${px(snapshot.price)}</div>
          <div className={snapshot.priceChange24hPct >= 0 ? "font-mono text-xs text-emerald-400" : "font-mono text-xs text-rose-400"}>
            {pct(snapshot.priceChange24hPct)}
          </div>
          <div className="hidden gap-3 text-[11px] text-zinc-400 md:flex">
            <span>Spread <b className="font-mono text-zinc-200">{snapshot.spreadBps.toFixed(1)}bps</b></span>
            <span>Funding <b className="font-mono text-amber-300">{pct(snapshot.fundingRate * 100, 4)}</b></span>
            <span>Regime <b className="text-sky-300">{snapshot.regime}</b></span>
            <span>Heat <b className="font-mono text-emerald-300">{snapshot.risk.heatPct.toFixed(1)}%</b></span>
            <span>Equity <b className="font-mono text-zinc-200">{usd(101_330, { compact: true })}</b></span>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <span className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase ${snapshot.connected ? "bg-emerald-500/10 text-emerald-300" : "bg-rose-500/10 text-rose-300"}`}>
              {snapshot.connected ? "Live" : "Offline"}
            </span>
            {criticalAlerts > 0 ? (
