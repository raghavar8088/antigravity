...
/**
 * NIFTY 50 option selling desk UI — layout and components aligned with
 * OptionsSellingScalper (BTC option selling) for a consistent workspace.
 */
...
function fmtLabel(name: string, id: number | undefined, map: StrategyNumberMap) {
  ...
  return n ? `${n}. ${name.replace(/_/g, " ")}` : name.replace(/_/g, " ");
}

// ── Design primitives (match BTC option selling) ────────────────────────────
...
function ExitBadge({ reason }: { reason: string }) {
  const map: Record<string, string> = {
    ...
    PROFIT_LOCK:  "border-blue-200 bg-blue-50 text-blue-700",
    TRAIL_STOP:   "border-amber-200 bg-amber-50 text-amber-700",
    LATE_EXIT:    "border-sky-200 bg-sky-50 text-sky-700",
  };
  return (
...
function PremiumBar({ entry, current }: { entry: number; current: number }) {
  ...
  const profitable = current <= entry;
  return (
    <div className="w-full max-w-[80px]">
      <div className="h-1.5 w-full rounded-full overflow-hidden" style={{ background: "var(--border)" }}>
        <div
          className={`h-full rounded-full transition-all ${profitable ? "bg-emerald-500" : "bg-rose-500"}`}
    ...
    <div className="glass-panel px-5 py-6 md:px-6">
      ...
      {positions.length === 0 ? (
        <div className="flex min-h-[180px] items-center justify-center rounded-[20px] border border-dashed px-6 py-12 text-center text-sm"
          ...
          No open short positions yet — the engine fills live slots on each tick against the NIFTY 50 index stream.
        </div>
      ) : (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-medium uppercase tracking-[0.12em]" style={{ color: "var(--text-secondary)" }}>
                {callCount} short calls | {putCount} short puts
              </span>
            </div>
            ...
            <table className="w-full text-left text-sm" style={{ minWidth: 920 }}>
              <thead style={{ background: "var(--surface-2)", color: "var(--text-secondary)" }}>
                <tr className="text-[11px] uppercase tracking-[0.12em]">
                  <th className="px-4 py-3 font-medium">Short position</th>
                  <th className="px-4 py-3 font-medium">Strike</th>
                  <th className="px-4 py-3 font-medium">Premium received</th>
                  <th className="px-4 py-3 font-medium">Opened</th>
                  <th className="px-4 py-3 font-medium">Greeks</th>
                  <th className="px-4 py-3 font-medium">PnL</th>
                  <th className="px-4 py-3 font-medium text-right">Decay</th>
                </tr>
              </thead>
              <tbody>
                {positions.map((pos) => (
                      ...
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>
                            {fmtLabel(pos.strategyName, pos.strategyId, strategyNumbers)}
                          </span>
                        </div>
                        <div className="text-[11px]" style={{ color: "var(--text-secondary)" }}>
                          Qty {fmt(pos.quantity, 0)} · Margin held {fmtINR(pos.costBasis)}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-xs">
                      <div className="font-mono" style={{ color: "var(--text-primary)" }}>{fmt(pos.strike, 0)}</div>
                      <div style={{ color: "var(--text-secondary)" }}>NIFTY {fmtINR(pos.entryBtcPrice, { decimals: 0 })}</div>
                    </td>
                    <td className="px-4 py-3 text-xs">
                      <div className="font-mono" style={{ color: "var(--text-primary)" }}>Sold {fmtINR(pos.entryPremium)}</div>
                      <div style={{ color: "var(--text-secondary)" }}>Now {fmtINR(pos.currentPremium)}</div>
                    </td>
                    <td className="px-4 py-3 text-xs">
                      {pos.entryTime ? (
                        <div>
    ...
    <div className="glass-panel px-5 py-6 md:px-6">
      <div className="mb-5 flex items-center justify-between gap-3">
        <h2 style={{ fontFamily: "var(--font-display)", fontSize: 11, fontWeight: 800, letterSpacing: "0.14em", color: "var(--text-secondary)" }}>
          SELLING STRATEGIES — LEADERBOARD ({totalStrategies})
        </h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left" style={{ minWidth: 1180 }}>
          <thead>
          ...
          <tbody>
            {visible.map((s, i) => (
              <tr key={`${s.name}-${s.strategyId}`} className="border-b transition-colors hover:bg-black/[0.015]" style={{ borderColor: "var(--border-subtle)" }}>
                <td className="py-2.5 px-3 text-xs font-mono" style={{ color: "var(--text-muted)" }}>{resolveNum(s.name, s.strategyId, strategyNumbers) || i + 1}</td>
                <td className="py-2.5 px-3">
                  <div className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>{fmtLabel(s.name, s.strategyId, strategyNumbers)}</div>
                ...
                <td className="py-2.5 px-3 text-sm font-mono" style={{ color: "var(--text-secondary)" }}>
                  ...
                  <div className={`${s.shadowPnl >= 0 ? "text-emerald-600" : "text-rose-600"} text-[11px] font-semibold`}>
                    {s.shadowTrades > 0 ? fmtINR(s.shadowPnl, { signed: true }) : "-"}
                  </div>
                </td>
                <td className="py-2.5 px-3 text-right text-sm font-mono" style={{ color: "var(--text-secondary)" }}>
                  {s.rosterState === "ACTIVE" ? fmtINR(s.allocationUsd) : "-"}
                </td>
                <td className="py-2.5 px-3 text-right text-sm font-mono" style={{ color: "var(--text-secondary)" }}>
    ...
    <div className="glass-panel px-5 py-6 md:px-6">
      ...
      {trades.length === 0 ? (
        <div className="rounded-2xl border border-dashed py-12 text-center" style={{ borderColor: "var(--border)", color: "var(--text-muted)", fontSize: 13 }}>
          No completed option writing trades yet.
        </div>
      ) : (
        <div className="space-y-4">
            ...
            <table className="w-full text-left text-sm" style={{ minWidth: 1040 }}>
              <thead style={{ background: "var(--surface-2)", color: "var(--text-secondary)" }}>
                <tr className="text-[11px] uppercase tracking-[0.12em]">
                  ...
                  <th className="px-4 py-3 font-medium">Contract written</th>
                ...
                {visibleTrades.map((t) => (
                  <tr key={t.id} className="border-t" style={{ borderColor: "var(--border-subtle)" }}>
                    ...
                    <td className="px-4 py-3 text-xs">
                      ...
                      <div style={{ color: "var(--text-secondary)", marginTop: 4 }}>{t.expiryMins}m expiry</div>
                    </td>
                    <td className="px-4 py-3 text-xs">
                      <div className="font-mono" style={{ color: "var(--text-primary)" }}>Sold {fmtINR(t.entryPremium)}</div>
                      <div style={{ color: "var(--text-secondary)" }}>Closed {fmtINR(t.exitPremium)}</div>
                    </td>
                    <td className="px-4 py-3 text-xs">
                      <div className="font-mono" style={{ color: "var(--text-primary)" }}>
                        {fmtINR(t.entryBtcPrice, { decimals: 0 })} → {fmtINR(t.exitBtcPrice, { decimals: 0 })}
                      </div>
                      <div style={{ color: "var(--text-secondary)" }}>Qty {fmt(t.quantity, 0)}</div>
                    </td>
...
export default function Nifty50OptionSellingScalper({
  ...
  const callCount = positions.filter((p) => p.optionType === "CALL").length;
  const putCount = positions.filter((p) => p.optionType === "PUT").length;
  const exposureSummary = openCount === 0 ? "No open exposure" : `${callCount} short calls / ${putCount} short puts`;
  ...
  const strategyNumbers = strategies.reduce<StrategyNumberMap>((map, s, i) => {
    map[s.name] = s.strategyId > 0 ? s.strategyId : i + 1;
    return map;
  }, {});
  ...
  const feedDetail = barCount >= 12 ? `${barCount} bars analyzed` : `Warming ${barCount}/12`;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 2xl:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)] items-start gap-5">
        ...
        <div className="glass-panel relative overflow-hidden px-6 py-7 md:px-7">
          ...
          <div className="flex flex-col gap-5">
            <div className="px-1">
              <div className="text-[10px] font-semibold uppercase tracking-[0.22em] text-zinc-500">
                NIFTY 50 OPTION WRITING (SELLING) EQUITY
              </div>
              ...
              <div className="mt-2 px-0.5 text-sm" style={{ color: "var(--text-secondary)" }}>
                Session PnL {fmtINR(sessionPnl, { signed: true })} · Premium collected · Spot{" "}
                <span className="font-mono font-medium" style={{ color: "var(--text-primary)" }}>
                  {displayedEnginePrice > 0 ? fmtINR(displayedEnginePrice, { decimals: 2 }) : "—"}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-1">
              <div className="flex flex-wrap gap-2">
                <BadgePill label="Selling engine online" tone="positive" />
                <BadgePill label={`${activeStrategies}/${totalStrategies} live`} tone="info" />
                <BadgePill label="Theta decay" tone="warning" />
                <BadgePill label="Premium writer" tone="neutral" />
                <BadgePill
          ...
          <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
            <CompactMetric label="Session runtime" value={sessionRuntime} detail={`${activeStrategies} strategies funded`} accent="text-zinc-900" />
            <CompactMetric
              label="Last closed trade"
              value={latestTrade ? fmtINR(latestTrade.netPnl, { signed: true }) : "No exits yet"}
              detail={latestTrade ? `${fmtLabel(latestTrade.strategyName, latestTrade.strategyId, strategyNumbers)} | ${latestTrade.exitReason}` : "Waiting for first premium decay cycle"}
              accent={latestTrade ? (latestTrade.netPnl >= 0 ? "text-emerald-600" : "text-rose-600") : "text-zinc-900"}
            />
            <CompactMetric label="Open exposure" value={exposureSummary} detail={`${openCount} of ${activeStrategies} live slots in position`} accent="text-zinc-900" />
          </div>
            ...
            return (
              <div className="mt-4 px-1">
                <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500 mb-2">
                  Premium equity curve · {trades.length} trades
                </div>
        ...
        <div className="glass-panel px-5 py-6 md:px-6">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <CompactMetric label="Selling equity" value={fmtINR(equity, { decimals: 0 })} detail={`Base ${fmtINR(INITIAL_BALANCE, { decimals: 0 })}`} accent="text-zinc-900" />
            <CompactMetric label="Net PnL" value={fmtINR(sessionPnl, { signed: true })} detail={`${totalReturnPct >= 0 ? "+" : ""}${totalReturnPct.toFixed(2)}% vs base`} accent={sessionPnl >= 0 ? "text-emerald-600" : "text-rose-600"} />
            <CompactMetric label="Closed PnL" value={fmtINR(closedPnl, { signed: true })} detail={`${totalTrades} completed writes`} accent={closedPnl >= 0 ? "text-emerald-600" : "text-rose-600"} />
            <CompactMetric label="Open positions" value={`${openCount}`} detail={`${exposureSummary} · ${feedDetail}`} accent="text-blue-600" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-7 gap-4">
        ...
        <SummaryCard label="Best trade" value={bestTrade ? fmtINR(bestTrade.netPnl, { signed: true }) : "-"} accent={bestTrade && bestTrade.netPnl >= 0 ? "text-emerald-600" : "text-rose-600"} />
      ...
      {bestStrategy && (
        <div className="glass-panel px-6 py-5 flex flex-wrap items-center gap-6 justify-between">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.18em]" style={{ color: "var(--text-muted)" }}>Top selling strategy</div>
            <div className="mt-1 text-lg font-bold" style={{ color: "var(--text-primary)" }}>{fmtLabel(bestStrategy.name, bestStrategy.strategyId, strategyNumbers)}</div>
          ...
          <div className="text-right">
            ...
            <div className={`mt-1 text-2xl font-bold ${bestStrategy.totalPnl >= 0 ? "text-emerald-600" : "text-rose-600"}`}>{fmtINR(bestStrategy.totalPnl, { signed: true })}</div>
          </div>
      ...
      <div className="text-center text-[11px]" style={{ color: "var(--text-muted)" }}>
        NIFTY options writing (selling) paper account · Client-side engine · ₹10,00,000 starting capital · Theta-first exits · Inverted PnL vs buy-side desk
      </div>
    </div>
  );
}
