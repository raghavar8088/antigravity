 ...
 *
 * A thin wrapper around OptionsScalper that wires it to the
 * /api/options-selling/* endpoints instead of /api/options/*.
 *
 * We do this by cloning the OptionsScalper component with the
 * selling hook and a different title/footer label so everything
 * is clearly presented as the "Selling (Writing) Engine".
 */
...
// For selling: premium decreasing = profit (inverted bar)
function PremiumBar({ entry, current }: { entry: number; current: number }) {
  // For seller: profit when current < entry => show green when declining
  const pct = entry > 0 ? Math.min(200, Math.max(0, (current / entry) * 100)) : 0;
  const profitable = current <= entry; // seller profits when premium decays
  return (
              ...
              <tbody>
                {positions.map((pos) => (
                  <tr key={pos.id} className="border-t" style={{ borderColor: "var(--border-subtle)" }}>
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="text-[11px]" style={{ color: "var(--text-secondary)" }}>
                          Qty {fmt(pos.quantity, 4)} | Premium received {fmtUSD(pos.costBasis)}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-xs">
                      <div className="font-mono" style={{ color: "var(--text-primary)" }}>Sold ${fmt(pos.entryPremium)}</div>
...
function DailyPnlPanel({ trades }: { trades: OptionTrade[] }) {
  return (
    <DailyPnlLedger
      ...
      description="Realized option-selling PnL grouped by trade exit day, with daily return based on that day's starting equity."
      emptyMessage="No closed option-selling trades yet, so there is no daily PnL ledger to display."
              ...
              <tbody>
                {visibleTrades.map((t) => (
                  <tr key={t.id} className="border-t" style={{ borderColor: "var(--border-subtle)" }}>
                    ...
                    <td className="px-4 py-3 text-xs">
                      <div className="font-mono" style={{ color: "var(--text-primary)" }}>Sold ${fmt(t.entryPremium)}</div>
                      <div style={{ color: "var(--text-secondary)" }}>Closed ${fmt(t.exitPremium)}</div>
                    </td>
                    <td className="px-4 py-3 text-xs">
                      <div className="font-mono" style={{ color: "var(--text-primary)" }}>
                        {fmtUSD(t.entryBtcPrice)} {"→"} {fmtUSD(t.exitBtcPrice)}
                      </div>
                      <div style={{ color: "var(--text-secondary)" }}>Qty {fmt(t.quantity, 2)}</div>
                    </td>
...
export default function OptionsSellingScalper({ actionsEnabled = false }: Props) {
  ...
  const handleReset = async () => {
    ...
    if (!confirm("Reset the options selling paper account to $1,000? All history will be cleared.")) return;
    setIsResetting(true);
    clearAll();
    try {
      const response = await fetch(`${API_URL}/api/options-selling/reset`, { method: "POST" });
      ...
      <div className="grid grid-cols-1 2xl:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)] items-start gap-5">
          ...
          <div className="flex flex-col gap-5">
            <div className="px-1">
              <div className="text-[10px] font-semibold uppercase tracking-[0.22em] text-zinc-500">
                BTC OPTION WRITING (SELLING) EQUITY
              </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-1">
              <div className="flex flex-wrap gap-2">
                ...
                <BadgePill label="Theta Decay Strategy" tone="warning" />
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button
                  ...
                  onClick={async () => {
                    ...
                    clearAll();
                    await fetch(`${API_URL}/api/options-selling/clear-history`, { method: "POST" });
                    setRefreshKey((k) => k + 1);
                  }}
                >
                  Clear Trades
        ...
        <div className="glass-panel px-5 py-6 md:px-6">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <CompactMetric label="Selling Equity" value={fmtUSD(equity)} detail={`Base ${fmtUSD(INITIAL_OPTIONS_BALANCE)}`} accent="text-zinc-900" />
            <CompactMetric label="Net PnL" value={fmtUSD(sessionPnl, { signed: true })} detail={`${totalReturnPct >= 0 ? "+" : ""}${totalReturnPct.toFixed(2)}% vs base`} accent={sessionPnl >= 0 ? "text-emerald-600" : "text-rose-600"} />
            <CompactMetric label="Closed PnL" value={fmtUSD(closedPnl, { signed: true })} detail={`${totalTrades} completed writes`} accent={closedPnl >= 0 ? "text-emerald-600" : "text-rose-600"} />
            <CompactMetric label="Open Positions" value={`${openCount}`} detail={exposureSummary} accent="text-blue-600" />
          </div>
        </div>
      </div>
